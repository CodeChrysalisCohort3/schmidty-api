...
describe('users', () => {
  describe('setup', () => {
    it('has run the initial migrations', () => knex('users').select()
  ...
  describe('#create', () => {
    ...
    context('when bad params are given', () => {
      ...
      it('politely refuses', () => db.users.create(params)
        ...
        .catch(err => expect(err.message).to.equal('Name must be provided, and be at least two characters')));
    });
    ...
    context('when good params are given', () => {
      ...
      it('creates a user', () => db.users.create(params)
        .then((user) => {
      ...
      context('when a duplicate name is provided', () => {
        ...
        it('generates a sanitized error message', () => db.users.create(params)
          .then(forcePromiseReject)
          .catch(err => expect(err.message).to.equal('The name already exists')));
      });
    });
  });
  ...
  describe('#list', () => {
    ...
    it('lists all users', () => db.users.list()
      .then((response) => {
        expect(usernames).to.include(response[0].username);
        expect(usernames).to.include(response[1].username);
      }));

    it('returns serializable objects', () => db.users.list()
      .then((response) => {
        expect(response[0].serialize).to.be.a('function');
        expect(response[0].serialize().id).to.be.a('number');
        expect(response[0].serialize().username).to.be.a('string');
      }));
  });
});
...
describe('channels', () => {
  describe('#create', () => {
    ...
    it('creates a channel', () => db.channels.create(params)
      .then((channel) => {
    ...
    context('when the name is upper cased', () => {
      it('forces the name to lower case', () => db.channels.create({ name: 'BIG CHANNEL' })
        .then((channel) => {
    ...
    context('when a duplicate name is provided', () => {
      ...
      it('generates a sanitized error message', () => db.channels.create(params)
        .then(forcePromiseReject)
        .catch(err => expect(err.message).to.equal('The name already exists')));
    });
  });
  ...
  describe('#list', () => {
    ...
    it('lists all channels', () => db.channels.list()
      .then((resp) => {
        expect(channelNames).to.include(resp[0].name);
        expect(channelNames).to.include(resp[1].name);
      }));

    it('returns serializable objects', () => db.channels.list()
      .then((resp) => {
...
describe('channel_messages', () => {
  ...
  before(() => db.users.create({ username: 'rp-3' })
    .then((user) => {
      fromId = user.id;
  ...
  describe('#create', () => {
    ...
    it('creates a message', () => db.channelMessages.create({ fromId, channelId, message })
      .then((messages) => {
        expect(messages).to.include({ fromUser: 'rp-3', toChannel: 'general', message });
  ...
  describe('#list', () => {
    before(() => knex('channel_messages').insert([
      { from_id: fromId, channel_id: channelId, message },
      { from_id: fromId, channel_id: otherChannelId, message },
    ])
      .then(() => Promise.delay(500))
      .then(() => knex('channel_messages').insert({
        from_id: fromId, channel_id: channelId, message: 'booya!',
      })));

    after(() => knex('channel_messages').del());

    it('lists the right number of messages', () => db.channelMessages.list({ channelId })
      .then((messages) => {
        expect(messages.length).to.equal(2);
      }));

    it('lists the right messages', () => db.channelMessages.list({ channelId })
      .then((messages) => {
        expect(messages[0]).to.include({ fromUser: 'rp-3', toChannel: 'general', message });
        expect(messages[0].id).to.be.a('number');
      }));

    it('lists message in the right order', () => db.channelMessages.list({ channelId })
      .then((messages) => {
...
describe('user_messages', () => {
  ...
  describe('#create', () => {
    ...
    it('creates a message', () => db.userMessages.create({ fromId, toId, message })
      .then((messages) => {
        expect(messages).to.include({ fromUser: 'rp-3', message });
        expect(messages.id).to.be.a('number');
        expect(messages.sentAt).to.be.a('Date');
      }));
  });
  ...
  describe('#list', () => {
    before(() => knex('user_messages').insert([
      { from_id: fromId, to_id: toId, message },
      { from_id: fromId, to_id: otherToId, message },
    ])
      .then(() => Promise.delay(500))
      .then(() => knex('user_messages').insert({
        from_id: toId, to_id: fromId, message: 'booya!',
      })));

    after(() => knex('user_messages').del());

    it('lists the right number of messages', () => db.userMessages.list({ fromId, toId })
      .then(messages => expect(messages.length).to.equal(2)));

    it('lists the right messages', () => db.userMessages.list({ fromId, toId })
      .then((messages) => {
        expect(messages[0]).to.include({ fromUser: 'rp-3', message });
        expect(messages[0].id).to.be.a('number');
        expect(messages[1]).to.include({ fromUser: 'muddybarefeet', message: 'booya!' });
        expect(messages[1].id).to.be.a('number');
      }));

    it('lists message in the right order', () => db.userMessages.list({ fromId, toId })
      .then((messages) => {
        expect(messages[0].message).to.equal(message);
        expect(messages[1].message).to.equal('booya!');
      }));
